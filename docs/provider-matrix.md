...
| Claude | Claude Code CLI | `~/.claude/settings.json`, `~/.claude/.credentials.json`, imported shared `AGENTS.md` plus `CLAUDE.md` overlay when present, `.mcp.json`, hooks | Partial | Hooks are guardrails, not the boundary; reviewed `.mcp.json` and auth can be injected explicitly |
