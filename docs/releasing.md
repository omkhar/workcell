...
publication and removes the admin-metadata credential before invoking the
...
The fresh final-gate design was live-certified on 2026-08-04 with signed tag
`v0.0.0-rc.4`. In one hosted-controls job and one shell step, the fixture used
its environment-scoped admin-metadata token to observe `enabled = true`, unset
that token, proved the default Actions token still received the exact HTTP 403
integration denial, and invoked the production publisher. [Workflow run
30955963114](https://github.com/omkhar/workcell-release-publisher-certification/actions/runs/30955963114)
