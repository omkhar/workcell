...
The collector requires a root `.git` directory, binds the physical worktree and Git directory, scrubs inherited shell/index/glob authority, and rejects nonregular or split-index state, shallow graphs, hidden flags, conversion filters, present gitlinks, and unsafe tracked ancestry.
