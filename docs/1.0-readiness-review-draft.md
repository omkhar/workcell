...
| D3 (complete) | verify-invariants static-invariant migration | Complete | `internal/workcellhardening/` + `cmd/workcell-citools`; the bulk of static invariants is in Go, with a documented narrow static residual accepted (not a completed migration) and a genuinely non-static tail that stays in bash by design — see the D3 (complete) status note in `docs/improvement-tracks-implementation-plan.md` |
| D6 | Split oversized Go validators | De-oversizing goal met | `internal/metadatautil/pinnedinputs_workflows.go`, `pinnedinputs_node.go`, and `pinnedinputs_docker.go` split out; the small Rust (~129 lines) and Python (~7 lines) validators remain inline as an accepted, non-oversized remainder (recorded 2026-07-09 in `docs/improvement-tracks-implementation-plan.md`) |
