...
func TestServerReportsMalformedRequest(t *testing.T) {
	server, client := localUnixConnectionPair(t)
	defer server.Close()
	defer client.Close()
	...
	if got := diagnostics.String(); got == "" || !bytes.Contains([]byte(got), []byte("read request")) {
...
func localUnixConnectionPair(t *testing.T) (*net.UnixConn, *net.UnixConn) {
	t.Helper()
