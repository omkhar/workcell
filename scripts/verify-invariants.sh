...
# A contiguous run of migrated checks is delegated through one
# `go_verify_citools workcell-check-batch "${ROOT_DIR}" <check>...` call: the
# subcommand runs them in argv order inside a single process and stops at the
# first failure, so the exit code and stderr are identical to running that
# check's own subcommand.
go_verify_citools() {
